...
public class FMColor : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        if (PaintBlurLevel > 0)
        {
            RenderTexture RTexture = RenderTexture.GetTemporary((source.width / (1+ PaintBlurLevel)), (source.height / (1+ PaintBlurLevel)), 0, source.format);
            ...
            {
                RenderTexture MainTex = RenderTexture.GetTemporary(RTexture.width, RTexture.height, 0, RTexture.format);
                Graphics.Blit(RTexture, MainTex, material, pass);
                //Graphics.Blit(RTexture, MainTex, material, (QualitySettings.activeColorSpace == ColorSpace.Linear ? 1 : 0));
                Graphics.Blit(MainTex, destination, FXAAMat);
                RenderTexture.ReleaseTemporary(MainTex);
            }
            else
            {
                //Graphics.Blit(RTexture, destination, material, QualitySettings.activeColorSpace == ColorSpace.Linear ? 1 : 0);
        ...
        else
        {
            ...
            if (FXAASettings.Enable)
            {
                RenderTexture MainTex = RenderTexture.GetTemporary(source.width, source.height, 0, source.format);
                //Graphics.Blit(source, MainTex, material, QualitySettings.activeColorSpace == ColorSpace.Linear ? 1 : 0);
